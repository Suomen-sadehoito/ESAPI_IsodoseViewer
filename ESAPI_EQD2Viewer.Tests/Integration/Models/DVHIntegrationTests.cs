    ...
    public class DVHIntegrationTests
    {
        ...
        public void EQD2DVHCurve_AllPoints_ShouldBeFiniteAndOrdered(string fixtureName)
        {
            ...
            foreach (var dvhFix in dvhFixtures)
            {
                ...
                // Build a DVHPoint array mimicking what DVHService receives
                var curveAsPoints = dvhFix.curve
                    .Select(p => new VMS.TPS.Common.Model.Types.DVHPoint(
                        new VMS.TPS.Common.Model.Types.DoseValue(p[0],
                ...
                foreach (double ab in new[] { 3.0, 10.0 })
                {
                    ...
                    {
                        double d = eqd2Curve[i].DoseValue.Dose;
                        double v = eqd2Curve[i].Volume;
                    ...
                    {
                        eqd2Curve[i].DoseValue.Dose.Should()
                            .BeGreaterOrEqualTo(eqd2Curve[i - 1].DoseValue.Dose - 1e-6,
